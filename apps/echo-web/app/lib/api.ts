...
const API_BASE_URL =
  ...
  'http://localhost:3001';
const DEMO_USER_ID =
  process.env.NEXT_PUBLIC_DEMO_USER_ID?.trim() ?? process.env.EXPO_PUBLIC_DEMO_USER_ID?.trim();
const DEMO_CREATOR_ID =
  process.env.NEXT_PUBLIC_DEMO_CREATOR_ID?.trim() ??
  process.env.EXPO_PUBLIC_DEMO_CREATOR_ID?.trim();
...
export const echoApi = {
  ...
  getSavedTracks(userId = DEMO_USER_ID) {
    if (!userId) {
      throw new Error('NEXT_PUBLIC_DEMO_USER_ID is required to load saved tracks');
    }
  ...
  getFollowedCreators(userId = DEMO_USER_ID) {
    if (!userId) {
      throw new Error('NEXT_PUBLIC_DEMO_USER_ID is required to load followed creators');
    }
  ...
  saveTrack(contentId: string, userId = DEMO_USER_ID) {
    if (!userId) {
      throw new Error('NEXT_PUBLIC_DEMO_USER_ID is required to save a track in the shell app');
    }
  ...
  followCreator(creatorId: string, userId = DEMO_USER_ID) {
    if (!userId) {
      throw new Error('NEXT_PUBLIC_DEMO_USER_ID is required to follow a creator in the shell app');
    }
  ...
  reportTrack(contentId: string, reportReason: ReportReason, reporterUserId = DEMO_USER_ID) {
    if (!reporterUserId) {
      throw new Error('NEXT_PUBLIC_DEMO_USER_ID is required to report a track in the shell app');
    }
